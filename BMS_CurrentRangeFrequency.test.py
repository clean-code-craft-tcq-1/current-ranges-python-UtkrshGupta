...
class CurrentRangeFrequencyTest(unittest.TestCase):
    ...
    def test_to_check_impact_if_cuurent_values_are_passed_as_string(self):
        data = [1,3,4,5,10,23,'2',13,14,19,15,18]
        crf = CurrentRangeFrequncy(data)
        self.assertRaises(TypeError, crf.get_current_ranges_frequency)
     
    def test_to_check_input_is_empty(self):
        crf = CurrentRangeFrequncy([])
        self.assertEqual(crf.get_current_ranges_frequency(), 'Empty Data')   
